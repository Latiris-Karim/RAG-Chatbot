...
def forgot_pw(email):
    ...
    if not res or not res[0]['email']:
        return {"message": "If the email exists, a reset link has been sent"}

    reset_token = secrets.token_urlsafe(32)
    expiry = datetime.now() + timedelta(minutes=30)
    store_reset_token(email, reset_token, expiry)
    return {"message": "Reset email sent", "token": reset_token, "email": email}
...
def reset_password(token, new_password):
    ...
    return change_pw(u_id, new_password)
...
def change_email_request(u_id, new_email):
    ...
    return {'message': 'Email change request has been submitted, check your new email', 'email': normalized_email, 'code': email_code}
